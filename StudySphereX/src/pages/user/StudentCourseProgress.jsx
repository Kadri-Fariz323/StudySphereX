...
export const StudentCourseProgress = () => {
  ...
  const progressData = studentCurrentCourseProgress?.progress || [];
  const quizProgress = studentCurrentCourseProgress?.quizProgress;
  const curriculum = courseDetails?.curriculum || [];

  const isCourseCompleted = useMemo(() => {
    if (!curriculum.length) return false;

    const completedCount = curriculum.filter((lecture) =>
      progressData.some(
  ...
  const isQuizPassed = quizProgress?.passed || false;
  ...
  async function updateCourseProgress() {
    if (!currentLecture || !auth?.user?._id) return;

    const isAlreadyViewed = progressData.some(
  ...
  }, [currentLecture?.progressValue, currentLecture?._id]);

  useEffect(() => {
    if (isCourseCompleted && !isQuizPassed && !showCourseCompleteDialog) {
    }
  }, [isCourseCompleted, isQuizPassed]);
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-inter text-slate-800">
      ...
      <div className="flex items-center justify-between p-4 bg-indigo-600 text-white border-b border-indigo-700 shadow-sm h-16 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold truncate max-w-[200px] sm:max-w-md">
            {courseDetails?.title || "Course Progress"}
          </h1>
        </div>
        <button
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div
          className={`flex-1 flex flex-col overflow-y-auto transition-all duration-300 ${isSideBarOpen ? "mr-0 lg:mr-0" : ""}`}
        >
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="space-y-1 p-2">
                {curriculum.map((item, index) => {
                  ...
                  const isWatched = progressData.some(
                    (prog) => prog.lectureId === item._id && prog.viewed,
                  );
              ...
              <div className="space-y-1 p-2 pb-10">
                ...
                >
                  <FileQuestion
                    className={`w-5 h-5 flex-shrink-0 ${isCourseCompleted ? "text-indigo-600" : ""}`}
                  />
                ...
                <div
                  onClick={() => {
                    if (isQuizPassed) navigate(`/course-certificate/${id}`);
                  }}
                  className={`
                       flex items-center gap-3 p-3 rounded-md transition-all
                       ${
                ...
                >
                  <Award
                    className={`w-5 h-5 flex-shrink-0 ${isQuizPassed ? "text-emerald-600" : ""}`}
                  />
                  <div className="flex-1">
                    <span className="text-sm font-medium block">
                      Course Certificate
          ...
          <DialogHeader>
            ...
            <DialogDescription className="flex flex-col gap-3 pt-2">
              <span className="text-sm text-gray-600">
                You have completed all lectures! You are now eligible for the
                final assessment.
              </span>
              <div className="flex flex-row gap-3 justify-end mt-2">
                ...
                <Button onClick={handleQuizNavigation}>Take Final Exam</Button>
              </div>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
